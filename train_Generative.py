...
ano_img = loadmat('./Data/tgt_cj.mat')#训练30 30 10
...
path = './Train/Results/spectral/Saved_models/HYDICE'
...
def encoder(x, reuse=False):
    ...
    with tf.name_scope('Encoder'):
        ...
        e_weights = tf.matmul(e_weights_1a,e_weights_2)
        #cam_variance = global_variance_pooling(latent_variable)
        #e_weights_2 = tf.multiply(cam_variance,e_weights)
        e_weights_2 = tf.sigmoid(e_weights_2)
        #latent_variable_1 = tf.multiply(cam_variance,latent_variable)
        latent_variable_1 = tf.multiply(e_weights_2,latent_variable)
...
def train(train_model=True):
    ...
    #dl_var = [var for var in all_variables if 'dl_' in var.name]
    # Optimizers
    ...
    #dl_optimizer = tf.train.RMSPropOptimizer(learning_rate=learning_rate).minimize(dl_loss, var_list=dl_var)
        ...
        if train_model:
            ...
            for i in range(n_epochs):
                ...
                for b in range(n_batches):
                    ...
                    decoder_path = './Train/Results/Spectral/Decoder/HYDICE/'
                    if i % 200 == 0:

                            scio.savemat(encoder_path+'feature%dout.mat'%(i),{'feature_%d'%(i):e_output})


                            scio.savemat(decoder_path+'decoder%dout.mat'%(i),{'decoder_%d'%(i):d_output})
            ...
            print('test')
            test_path = '.\Train\Results\Test_out\Cuploss_TD'
            saver.restore(sess, save_path=tf.train.latest_checkpoint('.\Train\Results\Spectral\Saved_models\HYDICE'))

            output_y = sess.run(decoder_output,feed_dict={x_input: data})
            """ input_y = []
            output_y = []
            n_batches = int(data.shape[0] / 1)
            print('n_batches',n_batches)
            for b in range(n_batches):
                index_1 = int(b * 1)
                index_2 = int(index_1 + 1)
                input_y = data[index_1:index_2,:]
                output_y = sess.run(decoder_output,feed_dict={x_input: input_y})
                output_y.append(b) """
